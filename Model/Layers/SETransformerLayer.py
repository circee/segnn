...
class SETransformerLayer(torch.nn.Module):
    ...
    def __init__(
        ...
        self.tp_k = FullyConnectedTensorProduct(input_irreps, edge_attr_irreps, hidden_irreps, shared_weights=False, normalization='component')
        self.fc_k = FullyConnectedNet([number_of_basis, 16, self.tp_k.weight_numel], act=torch.nn.functional.silu)

        self.tp_v = FullyConnectedTensorProduct(input_irreps, edge_attr_irreps, output_irreps, shared_weights=False, normalization='component')
        self.fc_v = FullyConnectedNet([number_of_basis, 16, self.tp_v.weight_numel], act=torch.nn.functional.silu)

        self.dot = FullyConnectedTensorProduct(hidden_irreps, hidden_irreps, "0e", normalization='component')
